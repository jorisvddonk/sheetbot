...
SELECT data.*,
  ((data.rust_surf IS NOT NULL AND data.lr_surf IS NOT NULL AND data.rust_surf != data.lr_surf) +
  (data.rust_atmo IS NOT NULL AND data.lr_atmo IS NOT NULL AND data.rust_atmo != data.lr_atmo) +
  (data.rust_pal IS NOT NULL AND data.lr_pal IS NOT NULL AND data.rust_pal != data.lr_pal) +
  (data.rust_sect_def_hm IS NOT NULL AND data.lr_sect_def_hm IS NOT NULL AND data.rust_sect_def_hm != data.lr_sect_def_hm) +
  (data.rust_sect_def_oc IS NOT NULL AND data.lr_sect_def_oc IS NOT NULL AND data.rust_sect_def_oc != data.lr_sect_def_oc) +
  (data.rust_sect_rand_hm IS NOT NULL AND data.lr_sect_rand_hm IS NOT NULL AND data.rust_sect_rand_hm != data.lr_sect_rand_hm) +
  (data.rust_sect_rand_oc IS NOT NULL AND data.lr_sect_rand_oc IS NOT NULL AND data.rust_sect_rand_oc != data.lr_sect_rand_oc) +
  (data.rust_sect_def_stex IS NOT NULL AND data.lr_sect_def_stex IS NOT NULL AND data.rust_sect_def_stex != data.lr_sect_def_stex) +
  (data.rust_sect_def_sky IS NOT NULL AND data.lr_sect_def_sky IS NOT NULL AND data.rust_sect_def_sky != data.lr_sect_def_sky) +
  (data.rust_sect_rand_stex IS NOT NULL AND data.lr_sect_rand_stex IS NOT NULL AND data.rust_sect_rand_stex != data.lr_sect_rand_stex) +
  (data.rust_sect_rand_sky IS NOT NULL AND data.lr_sect_rand_sky IS NOT NULL AND data.rust_sect_rand_sky != data.lr_sect_rand_sky)) AS errors,
