...
export class JsonSchemaWidget extends LitElement {
    ...
    explainSchema() {
      try {
        const schema = JSON.parse(this.data);
        const explanation = this.generateSchemaExplanation(schema);
        alert(explanation);
      } catch (e) {
    ...
    generateSchemaExplanation(schema, indent = '') {
      ...
      if (schema.not) {
        explanation += `${indent}This must NOT match: ${this.generateSchemaExplanation(schema.not, indent + '  ').trim()}\n`;
        return explanation;
      }
    ...
    explainObjectSchema(schema, indent) {
      ...
      if (schema.properties) {
        ...
        for (const [propName, propSchema] of Object.entries(schema.properties)) {
          ...
          explanation += `${indent}  - ${propName}${isRequired ? ' (required)' : ' (optional)'}: ${this.generateSchemaExplanation(propSchema, indent + '    ').trim()}\n`;
        }
      }
      ...
      if (schema.patternProperties) {
        ...
        for (const [pattern, propSchema] of Object.entries(schema.patternProperties)) {
          explanation += `${indent}  Properties matching "${pattern}" must match: ${this.generateSchemaExplanation(propSchema, indent + '    ').trim()}\n`;
        }
      }

      if (schema.propertyNames) {
        explanation += `${indent}Property names must match: ${this.generateSchemaExplanation(schema.propertyNames, indent + '  ').trim()}\n`;
      }
      ...
      if (schema.additionalProperties === false) {
        ...
        if (typeof schema.additionalProperties === 'object') {
          explanation += ` and must match: ${this.generateSchemaExplanation(schema.additionalProperties, indent + '  ').trim()}\n`;
        } else {
          explanation += '.\n';
        }
      }
      ...
      if (schema.dependencies) {
        ...
        for (const [prop, deps] of Object.entries(schema.dependencies)) {
          if (Array.isArray(deps)) {
            ...
            explanation += `${indent}  If ${prop} is present, it must match: ${this.generateSchemaExplanation(deps, indent + '    ').trim()}\n`;
          }
        }
      }
      ...
      if (schema.dependentSchemas) {
        ...
        for (const [prop, depSchema] of Object.entries(schema.dependentSchemas)) {
          explanation += `${indent}  If ${prop} is present, the data must also match: ${this.generateSchemaExplanation(depSchema, indent + '    ').trim()}\n`;
        }
      }
    ...
    explainArraySchema(schema, indent) {
      ...
      if (schema.items) {
        if (Array.isArray(schema.items)) {
          ...
          schema.items.forEach((itemSchema, index) => {
            explanation += `${indent}  Position ${index}: ${this.generateSchemaExplanation(itemSchema, indent + '    ').trim()}\n`;
          });
        } else {
          explanation += `${indent}All items: ${this.generateSchemaExplanation(schema.items, indent + '  ').trim()}\n`;
        }
      }
      ...
      if (schema.additionalItems !== undefined && Array.isArray(schema.items)) {
        if (schema.additionalItems === false) {
          ...
          explanation += `${indent}Additional items must match: ${this.generateSchemaExplanation(schema.additionalItems, indent + '  ').trim()}\n`;
        }
      }
    ...
    explainAllOfSchema(schema, indent) {
      ...
      schema.allOf.forEach((subSchema, index) => {
        explanation += `${indent}  Requirement ${index + 1}: ${this.generateSchemaExplanation(subSchema, indent + '    ').trim()}\n`;
      });
    ...
    explainOneOfSchema(schema, indent) {
      ...
      schema.oneOf.forEach((subSchema, index) => {
        explanation += `${indent}  Option ${index + 1}: ${this.generateSchemaExplanation(subSchema, indent + '    ').trim()}\n`;
      });
    ...
    explainConstraintKeywords(schema, indent) {
      ...
      // Array constraints
      if (schema.contains) {
        explanation += `${indent}Must contain at least one item matching: ${this.generateSchemaExplanation(schema.contains, indent + '  ').trim()}\n`;
      }
    ...
    explainConditionalSchema(schema, indent) {
      ...
      if (schema.if) {
        explanation += `${indent}If the data matches: ${this.generateSchemaExplanation(schema.if, indent + '  ').trim()}\n`;
      }

      if (schema.then) {
        explanation += `${indent}Then it must also match: ${this.generateSchemaExplanation(schema.then, indent + '  ').trim()}\n`;
      }

      if (schema.else) {
        explanation += `${indent}Otherwise it must match: ${this.generateSchemaExplanation(schema.else, indent + '  ').trim()}\n`;
      }
