...
if os.environ.get("SHEETBOT_AUTH_USER") and os.environ.get("SHEETBOT_AUTH_PASS"):
    auth_response = requests.post(SHEETBOT_BASEURL + "/login", json={
...
response = check_for_errors(requests.post(SHEETBOT_BASEURL + "/tasks/get", headers={**headers, "Content-Type": "application/json"}, json={"type": "python", "capabilities": capabilities}))
...
if "script" in json_data:
    ...
    os.environ["SHEETBOT_TASK_ACCEPTURL"] = SHEETBOT_BASEURL + "/tasks/" + json_data["id"] + "/accept"
    os.environ["SHEETBOT_TASK_COMPLETEURL"] = SHEETBOT_BASEURL + "/tasks/" + json_data["id"] + "/complete"
    os.environ["SHEETBOT_TASK_FAILEDURL"] = SHEETBOT_BASEURL + "/tasks/" + json_data["id"] + "/failed"
    os.environ["SHEETBOT_TASK_DATAURL"] = SHEETBOT_BASEURL + "/tasks/" + json_data["id"] + "/data"
    os.environ["SHEETBOT_TASK_ARTEFACTURL"] = SHEETBOT_BASEURL + "/tasks/" + json_data["id"] + "/artefacts"

    check_for_errors(requests.post(os.environ["SHEETBOT_TASK_ACCEPTURL"], headers={**headers, "Content-Type": "application/json"}, json={}))
    ...
    try:
        ...
        if '/// script' in script_code:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
                f.write(script_code)
                temp_file = f.name
            result = subprocess.run(['uv', 'run', '--script', temp_file], capture_output=True, text=True, env=os.environ.copy())
            os.unlink(temp_file)
        ...
        check_for_errors(requests.post(os.environ["SHEETBOT_TASK_COMPLETEURL"], headers={**headers, "Content-Type": "application/json"}, json={"data": data}))
    except Exception as e:
        print(e)
        check_for_errors(requests.post(os.environ["SHEETBOT_TASK_FAILEDURL"], headers={**headers, "Content-Type": "application/json"}, json={}))